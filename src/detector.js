      ...
      const isCfChl = Boolean(
        window._cf_chl_opt ||
          document.querySelector(
            "#challenge-running, #challenge-stage, #cf-please-wait"
          )
      );
...
const collectTokens = async (page, store) => {
  const domTokens = await page
    .evaluate(() => {
      const t = window.__tokens__ || {};
      return {
  ...
  return {
    recaptchaV2: store.recaptchaV2 || domTokens.recaptchaV2 || null,
    recaptchaV3: store.recaptchaV3 || domTokens.recaptchaV3 || null,
    hcaptcha:    store.hcaptcha    || domTokens.hcaptcha    || null,
    turnstile:   store.turnstile   || domTokens.turnstile   || null,
  };
};

module.exports = { deepDetect, collectTokens };
